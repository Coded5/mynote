...
- Sends RFN (Route Failure Network) toward sender
...
- **Cons**
    ...
    - Congestion Window mightn ot reflect the transmission rate acceptable to the network (*tf does this mean*)

### TCP with ELFN
- Explicit link failure notification (ELFN)
- ELFN originated from ndoe that detect path break can do 2 ways:
    - Sending ICMP to destination and DUR (Destination unreachable) message to sender
    - Piggy-back on RERR message that is to be sent to sender
- When ELFN is received sender disable retransmission timers and enter "standby" state.
...
- **Pros**
    ...
    - Less depenent on routing protocol 
- **Cons**
    - If network is (temporarily) partioned -> periodic probing eating up the bandwitdh.
...
- Need ABR (associativity-based routing protocol) which is not widely acceptedA
> Not scalable
> 
...
- Have 4 State:
    ...
    - Loss
    - Disconnected
- If intermediate node think that the network is partioned, TCP sender changed to 
- Use excplicit congestion notification (ECN)
- Don't have to modify tradition TCP (Is a thin layer between network layer and TCP Layer)
- **Pros**
    - Improve throughput
    - Maintain E2E  semantics
    - Is just additional layer of TCP
    - Work seamlessly with the internet
- **Cons**
    - Dependent on network layer protocol to detect the route change and partion
    - Need to update
...
- Motivation
    - Window based transmission: TCP relies on self-clocking, inducing burstiness of traffic which cause
    ...
    - Reliance on reverse path
        - TCP relies on ACK
        - In adhoc we use RTS, CTS and ACK in CSMA/CA making TCP ACKs kinda useless eating bandwitdth
- **Features**
    - No slow start (Estimate start-up rate)
    - Have progressive rate feedback for cogestion detection, avoidance, control

- **How**
    - Maintain sum of average queueing delay and average transmission delay:
