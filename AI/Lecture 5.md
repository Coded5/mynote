 ...
 *Like if-then statement with conditions and actions*
- Computer Program that simulate decision of **human expert** (Ex. Medicine)

    ### Components:
    - Knowledgebase: store knowledge in form of facts, rules and relationshis
...
*Prolog commonly use backward chaining*
...
*Explosion of the search space can cause some agent to loop indefinitely*

## Mycin

- *Mycin* (1970s) is classic expert system that use backward chaining to diagnose bacterial infection and recommended treatments
- Designed to reason under uncertainty, Have medical knowledge base.
- **Did not learn autonomously**
- Knowledgebase was built through *Knowledge Engineering* by human experts, adding facts, rules and relationships
    - Can be updated (by human) this is **Knowledge Acquisition not ML**.

# Agents

 **Terminologies**
 - **Rational**: Property of an agent that selects action to maximize expected utility given its percepts and knowledge.
...
- **AI Agent** is autonomous entity that perceives its environment through *sensory* input and select action base on the environment
...
- Need to formulate search problem must define:
    ...
    - *Path cost function* measures cost of a sequence of actions
- State space (s.k.a search space): set of all state with transitions between state that caused by actions
    - *Can be viewed as graph whose nodes are states and edges represent state transitions*
...
> Search explores the solution space to find paths while planning is the process of *searching* for actions that achieve the goal
...
- Represent problems as state space where state is a *configuration* of the problem (tf does configuration mean?)
...
- *Logic Theorist* (for proving mathematical theorem) GPS aimed to solve general problem across many domain.
    - *Use heuristic search*

> **Heuristic (Midterm!)**
> Strategies or Rules of Thumb that simplify decision-making by relying on *intuition* or *informed estimation*
